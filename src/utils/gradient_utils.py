# This file contains all the helper methods used to 
# generate adversarial images using the gradient ascent method
...
def generate_adversarial_dataset(data_directory, label_file, dest_directory,
    ...
    images, image_names = experiments.extractData(data_directory)
	
    for i in range (0,len(labels)):
        score_target = 0
        allowed_values = list(range(1, 1000+1))
        if labels[i] in allowed_values: allowed_values.remove(labels[i])
		
        while (score_target < 0.99):
            _, noisy_image, _, _, _, _, _, score_target = find_adversary_noise(session=session,
                                        y_pred=y_pred,
                                        ...
                                        + '/' + image_names[i],
                                        cls_target=random.choice(allowed_values))
        
        cv2.imwrite(expanduser(dest_directory) + '/' + image_names[i], noisy_image) 



def normalize_image(x):
    """This function normalizes the input so its values are between 0.0 and 1.0
     which is needed to show the noise properly."""
...
def adversary_example(session, y_pred, resized_image, gradient, pl_cls_target,
    ...
    image, noisy_image, noise, \
            name_source, name_target, \
            score_source, score_source_org, score_target = \
            find_adversary_noise(session=session,
                    y_pred=ypred,
                    resized_image=resized_image,
                    gradient=gradient,
                    pl_cls_target=pl_cls_target,
